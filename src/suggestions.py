...
Prefix lookup uses bisect on a sorted index instead of scanning every word,
so the vocabulary can grow without slowing down typing.

Everything you type is remembered in ~/.local/share/losk/words.json. No
internet, ever.
"""
...
hello hi hey help home however here please thanks thank sorry yes yeah nope
maybe okay ok sure right wrong true false correct incorrect definitely probably
absolutely certainly perhaps possibly obviously actually basically honestly
...
december spring summer autumn winter season holiday vacation birthday christmas
...
bring carry hold drop push pull turn shut lift lower throw catch
water food coffee tea drink eat sleep walk run drive ride fly buy sell pay
cook clean wash play sing dance travel visit meet call share follow join
...
neighbor cousin uncle aunt grandma grandpa partner roommate colleague
school work job money life world place thing time house street car road
city town store shop office building park garden kitchen bedroom
bathroom garage hospital library church market station airport hotel restaurant
...
anywhere everywhere nowhere sometime anytime somehow
great really very much many little few lot more less best better worse worst
enough almost quite rather pretty fairly extremely totally completely mostly
...
below inside outside near far away back forward together apart along across
should must might could would will can may shall ought
welcome goodbye bye cheers regards sincerely congratulations
question answer problem solution reason example idea plan project task goal
result purpose meaning detail point part piece section topic subject matter
number letter word line page book paper list note document report article
...
happy sad angry tired busy free ready sick fine well bad nice lovely
important interesting boring funny serious strange normal special common
red blue green yellow black white orange purple brown grey pink silver gold
one two three four five six seven eight nine ten eleven twelve twenty thirty
forty fifty hundred thousand million billion half quarter double triple
...
software program application system version feature option tool device
breakfast lunch dinner snack meal recipe table chair
bed sofa lamp door wall floor ceiling roof stairs yard fence
inbox outbox draft reply forward attach sender recipient
confirm cancel accept decline approve reject submit apply register
price cost cheap expensive discount sale offer deal budget
weather rain snow sun cloud wind storm hot cold warm cool dry wet
health doctor nurse medicine exercise sport team match score win
lose player coach field court ball race speed distance
flight train bus taxi ticket passport luggage map guide tour
course class lesson exam test grade degree college university
review publish comment post story
design develop launch release deploy fix improve
"""
...
does it|does not|did you|did not
...
please let|please send|please check|please make|please try|please confirm
...
is not|are not|was not|were not|has not|have not|will not
last week|next week|this week|last year|next year|this year
send me|send the|check the|open the|close the|save the|click the
"""
...
class SuggestionEngine:
    ...
    def vocabulary_size(self):
        return len(set(self.builtin) | set(self.learned))

    # ---------- fast prefix lookup ----------

    def _rebuild_index(self):
        """Sorted word list so prefix matches can be found with bisect instead
        of scanning the whole vocabulary on every keystroke."""
        self._index = sorted(set(self.builtin) | set(self.learned))

    def _prefix_matches(self, typed):
        start = bisect.bisect_left(self._index, typed)
        end = bisect.bisect_left(self._index, typed + "\uffff")
    ...
    def learn(self, word):
        ...
        if len(clean) < 2 or not clean.isalpha():
            self._previous = ""
            return
        is_new = clean not in self.learned and clean not in self.builtin
    ...
    def suggest(self, prefix, limit=6):
        ...
        for word in self._prefix_matches(typed):
            if word == typed:
                continue
            score = self.learned.get(word, 0) or self.builtin.get(word, 0)
            # A word that usually follows the previous word gets a big lift.
            score += followers.get(word, 0) * 4
